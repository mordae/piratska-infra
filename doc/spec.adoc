...
. Nad LDAPem je zprovozněn Keycloak ve stavu, který umožňuje registraci nových uživatelů a který respektuje výše uvedené schéma LDAPu. Nejsou povoleny způsoby registrace, které nevytvoří místní heslo v LDAPu.
...
. Nyní je možné měnit způsob ukládání hesel libovolně dle možností LDAPu a povolit v Keycloaku přihlašování pomocí služeb třetích stran, kdy nedojde k vytvoření místního hesla.
